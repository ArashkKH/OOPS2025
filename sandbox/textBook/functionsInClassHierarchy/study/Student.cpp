 ...
 * 4 - maintained to use inheritance concept
 * 5 - Added Polymorphism in Person class (using virtual functions and overiding them)
 */
...
GradStudent::GradStudent(): Student() , m_thesisScore(0.0){}

GradStudent::GradStudent(const char* name , const double* grades , int count, double thesisScore)
:Student(name , grades , count) , m_thesisScore(thesisScore){}

void GradStudent::display(ostream& os) const{
    Student::display(os);
    os << "Thesis Score: " << m_thesisScore << endl;
}

ostream& operator<<(ostream& os , const GradStudent& src){
    src.display(os);
    return os;
}

istream& operator>>(istream& is , GradStudent& src){
    Student& baseRef = src;
    is >> baseRef;

    cout << "Enter Thesis Score: ";
    is >> src.m_thesisScore;
    is.ignore();
...
void Student::display(ostream& os) const {
    ...
    os << "===================" << endl;
    os << "Average Score: " << getAverage() << endl;
}
...
istream& operator>>(istream& is , Student& src){
    ...
    src.memoryInit(int(strlen(name)) + 1,sz);
